...
import { RatingsInfo } from "@core/contracts/enums/service-orders/rating.info";
...
export const columns: Column<ServiceOrderDTO>[] = [
  ...
  {
    id: "satisfaction",
    accessorFn: (row) => row.satisfaction,
    title: "Avaliação",
    header: ({ column }) => (
      ...
      <div className="flex gap-2">
        <span className="max-w-[500px] truncate font-medium tabular-nums">
          {row.original.satisfaction ? (
            <StarRating
              rating={RatingsInfo[row.original.satisfaction.rating].score}
            />
          ) : (
            "Não avaliado"
          )}
